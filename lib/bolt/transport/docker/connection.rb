    ...
    class Docker < Base
      class Connection
        ...
        def write_remote_file_via_cp(source, destination)
          result = execute_local_docker_command('cp', [source, "#{container_id}:#{destination}"])
          raise "Error writing file to container #{@container_id}: #{result[1]}" unless result[2].exitstatus.zero?
        end
        ...
        def write_remote_directory_via_cp(source, destination)
          result = execute_local_docker_command('cp', [source, "#{container_id}:#{destination}"])
          raise "Error writing directory to container #{@container_id}: #{result[1]}" unless result[2].exitstatus.zero?
        end
